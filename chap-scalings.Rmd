...
In order to get a better idea of the variations with both latitude (from 0 to 90°) and elevation (from sea level to 3000 m) we can try the represent the evolution of the scaling factor for both paramters
...
:::: {.actionbox .actionimage data-latex="action"}
Explore the variations of the scaling factor by using various values for elevation `alt`, and different $R_c$ time-series. Look for the differences with the time-independant `st` scaling
::::
