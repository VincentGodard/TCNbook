...
We are going to use an example from @protin2019climatic, using sample *ARG-16-9*. 
...
compute_C<-function(t1,prm,P,lat,Lambda,S,Ss){
  ...
  Cmod = exp(-lambda*data$t1[nrow(data)])*trapz(data$t1,(prm[1,"Be10"]*data$lm+(prm[2,"Be10"] + prm[3,"Be10"])*S$Nmuons)*(exp(prm["lambda",'Be10']*data$t1)))
  return(Cmod)
}
#
