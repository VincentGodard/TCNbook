...
-   make some simple calculation of TCN concentration build up at the Earth surface, as a response to exposure and denudation
...
# Simple calcutions of TCN build up at the surface
...
Note that these often used SLHL values are defined for convenience, most calculator for exposure age work directly with the production rate value at calibration sites, and that they are always relative to the scaling scheme used (@borchers2016geological).
...
We are now going to use the `solv_conc_eul` function, which allows to easily deal with various configurations.
...
```{r ck_5bis}
# remember that t was defined in the previous code chunk
...
C2 = solv_conc_eul(z,ero,t,C0,prm[,"Be10"],S,Lambda) # compute concentration
plot(t/1000,C2,type="l",col="cornflowerblue",lwd=3,ylab="Concentration (at/g)",xlab="Time (ka)")
lines(t/1000,C1,lty=2,col="coral")
grid()
```

We start with zero erosion, that is the pure exposition of a surface.
...
-   Change the maximum of the `t` vector until you see the influence of radioactive decay.
-   Add some inheritance
-   Test the evolution with other nuclides (26Al and 14C)
...
We now compute the changes in concentration with depth $z$ along a profile.
